...
public class BillingExportBean {
	...
	ImportExportMsgAdminService importExportMsgAdminService;
	...
	private Collection getBillingProfileSummary() throws CUWBillingException, WSClientException, WSClientExceptionStatus{
		logger.debug( "getBillingProfileSummary");
		...
		logger.error( "getBillingProfileSummary");
		return summaryDTOs;
	...
	private ARTransactionSummaryDTO getBillingInvoiceSummary(BillingExportType invoiceType) throws CUWBillingException{
		logger.debug( "getBillingInvoicesSummary");
		logger.info("Entering getBillingInvoiceSummary method");
		PersistenceProvider provider = null;
		...
		logger.error( "getBillingInvoicesSummary");
		logger.info("Leaving getBillingInvoiceSummary method");
	...
	public void exportBillingProfiles(Date executeDate) throws CUWBillingException, BillingException{
		logger.debug( "exportBillingProfiles");
		...
		logger.error( "exportBillingProfiles");
	...
	private void exportBillingInvoices(BillingExportType invoiceType) throws CUWBillingException, BillingException{
		logger.debug( "exportBillingInvoices");
		logger.info("Entering exportBillingInvoices method");
		PersistenceProvider provider = null;
		String trackingNumber = UUIDUtil.getUUID();
		...
		logger.error( "exportBillingInvoices");
		logger.info("Exiting exportBillingInvoices method"); 
	 ...
	 * @param trackingNumber Allows correlation between a request and response
	 * @param successFlag Identifies whether the requested action was a success or failure
	 * @param description Description of the action/error that occurred
	 * @param messageType Enum defining type of message being logged
	...
	private void logImportExportNotification(String batchNumber, Boolean successFlag, String description, ImportExportMsgType messageType)
			throws CUWBillingException{
		logger.debug( "logImportExportNotification");
		...
		try{
			...
			importExportMsgAdminService.create(message);
		} catch (ImportExportMsgException e){
			throw new CUWBillingException(e.getMessage(), e);
		}
		logger.error( "logImportExportNotification");
	}
	/**
	...
	private Collection mapProfileDTOs(Collection profileDTOs) throws CUWBillingException, ParseException{
		...
		try {
			...
			logger.info(" CustomerExportTask last successful ExecutionDate : "+ lastExecutionDate);
		} catch (PersistenceProviderException e) {
			logger.error("Error while fetching the last successful execution date for CUWCustomerExportTask:"+ e.getMessage());
			throw new CUWBillingException("Error while fetching the last successful execution date for CUWCustomerExportTask:",	e);
		}
		if (profileDTOs != null){
			...
			while (it.hasNext()){
				...
				if(dto.getModifiedDate()!= null){
					...
					/*Send the records whose last Modified date 
					 * is greater than or equals to last successful execution date and 
