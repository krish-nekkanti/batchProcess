...
public class EventRouterDelegate{
	...
	public void send(Event event){
		try{
			logger.debug("[Notification] Event info: "
					+ "Date = " + event.getDate() 
					+ ", Event ID = " + ((AxiomEventType) event.getEventType()).getID()
			...
			exc.printStackTrace(); 
			logger.error("Error while sending message "+e)// todo:: log?
		}
	}
	/**
	 ...
	 * Constructs an event and sends event to the event router, which transforms
	 * event into the notification and distributes it to the appropriate
	 * subscribers. Additionally, this method provides ability to retrieve
	 * message from the resource bundle and format it with the provided
	 * parameters
	 * 
	 * This is helper method for <code>send( Event event )</code> method
	 * 
	...
	public void send(PersistenceProvider provider, AxiomEntity entity, AxiomEntityType type, AxiomEventCategory cat, String messageKey, String bundleName,
		...
		logger.debug("[Notification] Entity that triggered the event, "
				+ "  Entity ID = " + entity.getID() 
				+ ", Entity Name = " + entity.getName()
				+ ", Entity Type = " + type.getDisplayValue() 
				+ ", Event Category Type = " + cat.getDisplayValue());
